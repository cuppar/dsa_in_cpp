...
static std::istringstream input("{[hello](world!)}x[)]  [  [(foo)bar]begin end] begin ()end#");
...
void parser()
{
    ...
    while (currentToken != tok_eof)
    {
            ...
            currentToken == '}')
        {
            ...
            {
                reportError("except `", (char)pairMap[leftPair], "`, but get `end`");
            }
            if (currentToken == ')' && leftPair != '(')
            {
                if (leftPair == tok_begin)
                    reportError("export ", "`end`", ", but get `)`");
                else
